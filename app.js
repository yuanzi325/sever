...
async function toggleResolved(id){
  ...
  try {
    ...
    showToast('操作失败，请检查网络', null, false);
  }
}
...
const MemoryVaultBridge = {
  ...
  importState: async (text) => {
    ...
    if (r?.ok) showToast('已导入本地数据', null, false);
  },
...
async function doImport(mode){
  ...
  try {
    if (mode === 'merge') {
      ...
      const r = await saveAndRender(); closeEditor();
      window.__pendingImportData = null;
      if (r?.ok) showToast(`追加完成，新增 ${added} 条`, null, false);
      else showToast('追加保存失败，请检查网络', null, false);
    } else {
      await MemoryVaultBridge.importState(JSON.stringify(payload));
      closeEditor();
      window.__pendingImportData = null;
    }
...
async function restoreUndo(){
  ...
  if (undoPayload.type === 'memory') {
    ...
    undoPayload = null;
    renderAfterMemoryApiChange(); showToast('已撤销删除', null, false);
  } else {
...
async function restoreArchivedMemory(id){
  ...
  try {
    ...
    showToast('移出归档失败，请检查网络', null, false);
  }
}
