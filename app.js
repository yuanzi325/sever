...
const CORE_SUBLAYERS = {
  identity:'我，与我', preferences:'心事迷宫', rules:'温存之间', lore:'蝴蝶传讯'
};
const MOOD_COLORS = {
...
function normalizeState(raw){
  ...
  const ids = (raw?.memories || defaultState().memories).slice();
  const people = {yuan:'', ke:''};
  ids.forEach(m => {
    if (m.layer === 'core' && m.sub_layer === 'identity') {
      if (m.title === '沅沅') people.yuan = m.content || '';
      if (m.title === '小克') people.ke = m.content || '';
    }
  });
  if (!people.yuan || !people.ke) {
    const fallback = ids.find(m => m.layer === 'core' && m.sub_layer === 'identity');
    if (!people.yuan) people.yuan = fallback?.content || '';
    if (!people.ke) people.ke = '';
  }
  s.profile.people = {...{yuan:people.yuan, ke:people.ke}, ...(raw?.profile?.people || {})};
  s.profile.identityUpdatedAt = raw?.profile?.identityUpdatedAt || today();
...
const MemoryVaultBridge = {
  ...
  importState: (text) => {
    ...
    state = normalizeState(parsed); syncIdentityRecords(); persist(); renderAll();
...
function persist(snapshot = state){
  ...
  queueStateWriteToSupabase(snapshot);
}
function uid(prefix='id'){ return prefix + '_' + Math.random().toString(36).slice(2,9); }
function today(){ const d = new Date(); return d.toISOString().slice(0,10); }
function nowString(){ const d = new Date(); return d.toISOString().slice(0,16).replace('T',' '); }
...
function unreadCount(){ return state.bottles.filter(b => !b.read).length; }
...
function randomFloatMemory(){
  if (floatMemDate === today() && floatMemCache) return floatMemCache;
  const candidates = state.memories.filter(m => ['treasure','core','daily'].includes(m.layer));
  if (!candidates.length) return null;
  floatMemCache = candidates[dailySeed() % candidates.length];
  floatMemDate = today();
...
function refreshFloatMemory(){
  ...
  floatMemCache = candidates[Math.floor(Math.random()*candidates.length)];
  renderHome();
}
...
function syncIdentityRecords(){
  const yuanText = state.profile.people?.yuan || '';
  const keText = state.profile.people?.ke || '';
  const ensure = [
    {who:'沅沅', body:yuanText},
    {who:'小克', body:keText}
  ];
  ensure.forEach(({who, body}) => {
    const idx = state.memories.findIndex(m => m.layer==='core' && m.sub_layer==='identity' && m.title===who);
    const record = {id: idx>=0 ? state.memories[idx].id : uid('m'), layer:'core', sub_layer:'identity', title:who, content:body, date:today(), keywords:[who,'身份'], importance:5, mood:'平静', author:who};
    if (idx>=0) state.memories[idx] = {...state.memories[idx], ...record}; else state.memories.unshift(record);
  });
}
...
function coreSubNotesText(sub){
  const map = {
    all:`世界的果核分成四个子分区。
• identity — 我，与我：写清你是谁、我是谁，以及彼此如何称呼
...
function openIdentityModal(){
  closeModal();
  showEditor(`
    <div class="editor-header"><button class="editor-back" onclick="closeEditor()">←</button><div><div class="modal-title">我，与我</div><div class="subtitle">最近更新：${escapeHtml(state.profile.identityUpdatedAt || today())}</div></div></div>
    <div class="editor-main form-grid">
      <div class="note-box">这里用来整理你们彼此的身份、称呼和说话方式。保存后会同步到记忆里的“我，与我”。</div>
      <label class="input-shell"><span class="input-label">沅沅</span><textarea id="identity-yuan" placeholder="写沅沅是谁、她喜欢什么，以及希望怎样被称呼。">${escapeHtml(state.profile.people?.yuan || '')}</textarea></label>
      <label class="input-shell"><span class="input-label">小克</span><textarea id="identity-ke" placeholder="写小克是谁、他怎样和沅沅说话，以及他记得什么。">${escapeHtml(state.profile.people?.ke || '')}</textarea></label>
    </div>
    <div class="editor-actions"><button class="solid-btn" onclick="saveIdentityProfile()">保存</button><button class="ghost-btn" onclick="closeEditor()">取消</button></div>
  `);
}

function saveIdentityProfile(){
  state.profile.people = {
    yuan: document.getElementById('identity-yuan').value.trim(),
    ke: document.getElementById('identity-ke').value.trim()
  };
  state.profile.identityUpdatedAt = today();
  syncIdentityRecords();
  saveAndRender();
  showToast('身份信息已同步到记忆', null, false);
  closeEditor();
}
  ...
  document.getElementById('page-home').innerHTML = `
    ...
    <div class="section">
      <div class="card" style="background:var(--lmint)">
        ...
        <div class="small muted" style="margin-top:8px">预计下次周期：${cycle.next.toISOString().slice(0,10)} · 排卵期推测：${cycle.ovulation.toISOString().slice(0,10)}</div>
      </div>
    </div>
  `;
}
...
function renderMemory(){
  let list = state.memories.filter(m => MEMORY_FILTERS.includes(m.layer));
  ...
  if (searchText.trim()) {
    ...
    list = list.filter(m => [m.title,m.content,m.why_precious,m.today_snapshot,m.sub_layer,(m.keywords||[]).join(' ')].join(' ').includes(q));
  }
...
function memoryCard(m){
  ...
  return `
    <div class="mem-card" style="background:${meta.bg}" onclick="openMemoryDetail('${m.id}')">
      <div class="meta">
...
function renderBottle(){
  const list = sortByDateDesc(state.bottles, 'date');
...
function renderArchive(){
  ...
  document.getElementById('page-archive').innerHTML = `
    <div class="page-top">
      ...
      <div class="subtitle">这里只放回顾型内容：统计、日历、收藏夹、身份与设置。</div>
    </div>
    <div class="id-card">
    ...
    <div class="section">
      ...
      <div class="link-row" onclick="openIdentityModal()"><span>我，与我</span><span class="arrow">→</span></div>
    ...
    <div class="section">
      ...
      <div class="health-grid">
        <div class="health-card"><h4>下次预计时间</h4><div class="detail-body">${cycle.next.toISOString().slice(0,10)}</div><div class="archive-note" style="margin-top:8px">距现在 ${cycle.daysUntil} 天</div></div>
        <div class="health-card"><h4>排卵期推测</h4><div class="detail-body">${cycle.ovulation.toISOString().slice(0,10)}</div><div class="archive-note" style="margin-top:8px">周期 ${state.health.cycle.cycleLength} 天 / 经期 ${state.health.cycle.periodLength} 天</div></div>
      </div>
    </div>
  `;
}
...
function openMemoryDetail(id){
  ...
  const meta = layerMeta(m.layer);
  const detailTags = [meta.name, m.sub_layer ? CORE_SUBLAYERS[m.sub_layer] : '', m.mood || '', `importance ${m.importance}`, m.author || ''].filter(Boolean)
  ...
  showModal(`
    ...
    ${keywordRow}
    <div class="detail-body" style="margin-top:${(m.keywords||[]).length ? '14px' : '14px'}">${escapeHtml(m.content)}</div>
...
function openMemoryForm(id=''){
  const m = id ? state.memories.find(item => item.id === id) : {layer:'daily',sub_layer:'',title:'',content:'',date:today(),keywords:[],importance:2,mood:'平静',author:'小克',today_snapshot:'',why_precious:''};
  showEditor(`
    ...
    <div class="editor-main form-grid">
      ...
      <div class="editor-grid-three">
        ...
        <label class="input-shell"><span class="input-label">重要程度</span><select id="mf-importance">${[1,2,3,4,5].map(n=>`<option value="${n}" ${Number(m.importance)===n?'selected':''}>${n}</option>`).join('')}</select></label>
      </div>
  ...
  setTimeout(() => {
    ...
    layerSel?.addEventListener('change', () => {
      ...
      document.getElementById('subshell').style.display = val === 'core' ? 'block' : 'none';
...
function submitMemoryForm(id=''){
  const record = {
    id: id || uid('m'),
    layer: document.getElementById('mf-layer').value,
    ...
    importance: Number(document.getElementById('mf-importance').value || 2),
    mood: document.getElementById('mf-mood').value,
...
function openImportExport(){
  showEditor(`
    ...
    <div class="notice">当前所有记录都保存在浏览器本地。你可以导出整库，也可以按模块分别导出，再把改好的 JSON 贴回来导入。</div>
    ...
    <label class="input-shell" style="margin-top:14px"><span class="input-label">导入 JSON</span><textarea id="import-json" placeholder="把完整 JSON 粘贴到这里，再点下面导入。"></textarea></label>
    <div class="action-row"><button class="solid-btn" onclick="submitImportJson()">导入</button></div>
  `);
}

function copyExportJson(){ navigator.clipboard.writeText(JSON.stringify(state, null, 2)); showToast('JSON 已复制', null, false); }
function downloadExportJson(){
  const blob = new Blob([JSON.stringify(state, null, 2)], {type:'application/json'});
  const a = document.createElement('a');
  a.href = URL.createObjectURL(blob);
  a.download = 'memory-vault-data.json';
  a.click();
...
function downloadModuleExport(module='all'){
  ...
  const a = document.createElement('a');
  a.href = URL.createObjectURL(blob);
  a.download = `记忆库-${names[module] || module}.json`;
  a.click();
  URL.revokeObjectURL(a.href);
  showToast(`已导出 ${names[module] || module}`, null, false);
}
function submitImportJson(){
  const text = document.getElementById('import-json').value.trim(); if (!text) return;
  try { MemoryVaultBridge.importState(text); closeEditor(); }
  catch(e){ showToast('导入失败，请检查 JSON 格式', null, false); }
}
    ...
    <div class="editor-main form-grid">
      ...
      <div class="section">
        ...
        <div class="health-grid">
          ...
          <div class="health-card"><h4>下次预计</h4><div class="detail-body">${cycle.next.toISOString().slice(0,10)}</div></div>
          <div class="health-card"><h4>周期长度</h4><div class="detail-body">${state.health.cycle.cycleLength} 天</div></div>
          <div class="health-card"><h4>排卵期推测</h4><div class="detail-body">${cycle.ovulation.toISOString().slice(0,10)}</div></div>
        </div>
      </div>
      <div class="toolbar">
