...
function randomFloatMemory(){
  ...
  const candidates = state.memories.filter(m => ['treasure','core','daily'].includes(m.layer) && !m.resolved && !m._archived);
...
function syncIdentityRecords(){
  ...
  const ensure = [
    ...
    const prev = idx >= 0 ? state.memories[idx] : null;
    const base = normalizeMemoryMeta(prev || {id: uid('m'), layer:'core', sub_layer:'identity', title:who});
    const record = normalizeMemoryMeta({
      ...
      date:today(),
      keywords:[who,'身份'],
      importance:base.pinned ? (base.importance || 10) : Math.max(5, Number(base.importance || 5)),
      mood:'平静',
      author:who,
      protected:true
    });
    if (idx>=0) state.memories[idx] = record; else state.memories.unshift(record);
  });
}
...
function renderMemory(){
  let list = state.memories.filter(m => MEMORY_FILTERS.includes(m.layer) && !m._archived);
  ...
  try {
    if (mode === 'merge') {
      ...
      if (data.diaries && Array.isArray(data.diaries)){
        const diaryDates = new Set(state.diaries.map(d => d.date));
        for (const d of data.diaries){
          if (!diaryDates.has(d.date)){ state.diaries.push(d); added++; }
        }
      }
