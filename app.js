...
async function apiWriteMemory(record){
  ...
  if (!resp.ok) throw new Error('apiWriteMemory failed: ' + resp.status);
...
async function apiPatchMemory(id, fields){
  ...
  if (!resp.ok) throw new Error('apiPatchMemory failed: ' + resp.status);
  return normalizeMemoryMeta(await resp.json());
}
...
async function apiArchiveMemory(id){
  ...
  if (!resp.ok) throw new Error('apiArchiveMemory failed: ' + resp.status);
  return resp.json();
}
...
async function apiRestoreMemory(id){
  ...
  if (!resp.ok) throw new Error('apiRestoreMemory failed: ' + resp.status);
  return normalizeMemoryMeta(await resp.json());
}
...
async function readStateFromSupabase(){
  ...
  try{
    ...
    try {
      baseState.memories = await apiFetchMemories({ limit: 200, include_archived: true });
    } catch(e) {
...
function showConflictModal(){
  showModal(`
    ...
    <div class="action-row">
      ...
      <button class="solid-btn" data-action="conflict-force-save">用我的版本覆盖云端</button>
...
function openMemoryDetail(id){
  ...
  showModal(`
    ...
    <div class="action-row">
      <button class="solid-btn" data-action="open-memory-form" data-id="${escapeHtml(m.id)}">编辑</button>
      <button class="ghost-btn" data-action="toggle-resolved" data-id="${escapeHtml(m.id)}">${m.resolved ? '重新激活' : '标记已解决'}</button>
      <button class="ghost-btn" data-action="toggle-pinned" data-id="${escapeHtml(m.id)}">${m.pinned ? '取消钉选' : '钉选'}</button>
      <button class="danger-btn" data-action="delete-memory" data-id="${escapeHtml(m.id)}">删除</button>
...
function doImport(mode){
  ...
  try {
    if (mode === 'merge') {
      ...
      MemoryVaultBridge.importState(JSON.stringify(payload));
      closeEditor();
    }
    window.__pendingImportData = null;
...
async function deleteMemory(id){
  ...
  try {
    ...
    showToast('删除失败，请检查网络', null, false);
  }
}
...
async function restoreUndo(){
  ...
  if (undoPayload.type === 'memory') {
    try {
      const id = undoPayload.data?.id;
      if (id) await apiRestoreMemory(id);
      state.memories.unshift(undoPayload.data);
    } catch(err) {
      showToast('撤销失败，请检查网络', null, false);
      return;
    }
  } else {
...
document.body.addEventListener('click', (e) => {
  ...
  switch (action) {
    ...
    case 'open-memory-form': openMemoryForm(id); break;
    ...
    case 'delete-memory': deleteMemory(id); break;
    case 'open-diary-form': openDiaryForm(id); break;
    ...
    case 'open-bottle-form': openBottleForm(id); break;
...
async function loadStateForUser(){
  ...
  if (remote.ok) {
    ...
    remoteReadOk = true;
    try {
      state.memories = await apiFetchMemories({ limit: 200, include_archived: true });
    } catch(e) {
      remoteReadOk = false;
